-- Migration 0014: Document Templates and Lead Contract Fields

-- 1. Add contract fields to public.leads
ALTER TABLE public.leads
ADD COLUMN IF NOT EXISTS client_address text,
ADD COLUMN IF NOT EXISTS client_postal_code text,
ADD COLUMN IF NOT EXISTS client_city text,
ADD COLUMN IF NOT EXISTS client_province text,
ADD COLUMN IF NOT EXISTS vehicle_kms integer,
ADD COLUMN IF NOT EXISTS vehicle_color text,
ADD COLUMN IF NOT EXISTS vehicle_fuel text,
ADD COLUMN IF NOT EXISTS vehicle_reg_date text,
ADD COLUMN IF NOT EXISTS vehicle_itv_date text,
ADD COLUMN IF NOT EXISTS reservation_amount numeric(10,2);

-- 2. Create document_templates table
CREATE TABLE IF NOT EXISTS public.document_templates (
  id          text PRIMARY KEY, -- 'reserva_venta', 'contrato_compraventa', 'garantia_delegada'
  name        text NOT NULL,
  description text,
...
INSERT INTO public.document_templates (id, name, description, content)
VALUES
(
  'reserva_venta',
  'Reserva de Compra de Vehículo',
  'Documento oficial de formalización de reserva y señal económica para compraventa.',
  jsonb_build_object(
...
(
  'garantia_delegada',
  'Garantía Comercial Delegada',
  'Certificado de garantía comercial delegada de vehículo seminuevo.',
  jsonb_build_object(
    ...
    'general_conditions', jsonb_build_array(
      ...
      'En caso de que el periodo de garantía del fabricante fuese menor a 12 meses desde la firma de este documento, se pacta de común acuerdo que Rangel & Serrano se hará cargo las reparaciones según el Real Decreto-Ley 7/2021 hasta que se cumplan 12 meses desde la firma de este documento (excepto en los casos estipulados en las exclusiones). Estas reparaciones se realizarán en taller autorizado por Rangel & Serrano, S.L.',
...
(
  ...
  'Contrato de Compraventa y Garantía',
  'Contrato completo de compraventa y garantía de vehículo usado.',
  jsonb_build_object(
